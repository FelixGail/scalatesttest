...
/**
 * Copied from https://github.com/edadma/b-tree/blob/master/src/main/scala/BPlusTree.scala
 * ISC License (ISC)
 * Copyright 2016 Edward A. Maxedon, Sr.
 */
...
abstract class BPlusTree[K <% Ordered[K], +V] {
   ...
   */
  protected def getBranches( node: N ): scala.collection.Seq[N]
   ...
   */
  protected def getKeys( node: N ): scala.collection.Seq[K]
   ...
   */
  protected def getValues( node: N ): scala.collection.Seq[V]
  ...
  private def boundsPreprocess( bounds: scala.collection.Seq[(Symbol, K)], strictinf: K => (N, Int), inf: K => (N, Int) ) = {
