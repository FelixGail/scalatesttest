...
/**
 * Copied from https://github.com/edadma/b-tree/blob/master/src/main/scala/MemoryBPlusTree.scala
 * ISC License (ISC)
...
class MemoryBPlusTree[K <% Ordered[K], V](val order: Int) extends BPlusTree[K, V] {
  ...
  var first: Node[K, V] = new LeafNode[K, V](null)
  var last: Node[K, V] = first
  var root: Node[K, V] = first
  var lastlen = 0
