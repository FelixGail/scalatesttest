...
/**
 * Copied from https://github.com/aconley/Algorithms/blob/master/Sedgewick/Search/src/main/scala/RedBlackTree.scala
 * MIT License (MIT)
 *
 */
...
case class Tip[K, V]() extends RBT[K, V] // Black by convention
case class Branch[K, V](color: Color, left: RBT[K, V],
                        key: K, value: V, right: RBT[K, V]) extends RBT[K, V]

class RedBlackTree[K, V](implicit ord: Ordering[K]) extends SymbolTable[K, V] {

  var tree: RBT[K, V] = Tip[K, V]()
  ...
  def count(t: RBT[K, V]): Int = t match {
    case Tip() => 0
    case Branch(_, l, _, _, r) => 1 + count(l) + count(r)
  }

  def height(t: RBT[K, V]): Int = t match {
    case Tip() => 0
    case Branch(_, l, _, _, r) => 1 + Math.max(height(l), height(r))
  }
